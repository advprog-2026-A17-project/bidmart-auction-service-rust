...
fn docker_compose_routes_gateway_auction_traffic_to_rust_service() {
    ...
    assert!(auction_service_block.contains("CATALOGUE_SERVICE_URL: ${CATALOGUE_SERVICE_URL:-http://catalogue-service:8081}"));
    assert!(auction_service_block.contains("WALLET_SERVICE_URL: ${WALLET_SERVICE_URL:-http://wallet-service:8083}"));
